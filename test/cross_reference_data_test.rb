...
module TeamApi
  class CrossReferenceDataTest < ::Minitest::Test
    ...
    def test_item_to_xref
      ...
      xref_fields = %w(username quest avg_airspeed_of_an_unladen_swallow)
    ...
    def create_source_and_target_xrefs(source_data: nil, target_data: nil,
      ...
      [CrossReferenceData.new(site, 'source', ['username']),
       CrossReferenceData.new(site, 'target', ['username'])]
    end

    def test_empty_source_data
      source, target = create_source_and_target_xrefs(
        target_data: { 'bar' => { 'username' => 'bar' } })
    ...
    def test_missing_target_id_ignored_in_public_mode
      source, target = create_source_and_target_xrefs(
        source_data: { 'foo' => { 'username' => 'foo', 'target' => ['bar'] } },
        public_mode: true)
    ...
    def test_xref_source_and_target
      ...
      source.create_xrefs target
      assert_equal [{ 'username' => 'bar' }], source.data['foo']['target']
      assert_equal [{ 'username' => 'foo' }], target.data['bar']['source']
    end

    def test_xref_source_and_target_using_alternate_target_field_name
      source, target = create_source_and_target_xrefs(
        source_data: {
          'foo' => { 'username' => 'foo', 'alternate_name' => ['bar'] },
        },
        target_data: { 'bar' => { 'username' => 'bar' } })

      source.create_xrefs target, source_to_target_field: 'alternate_name'
      assert_equal [{ 'username' => 'bar' }], source.data['foo']['alternate_name']
      assert_equal [{ 'username' => 'foo' }], target.data['bar']['source']
    end
    ...
    def test_remove_duplicate_source_references_in_target_object
      source_data = {
        'foo' => { 'username' => 'foo', 'list_a' => ['bar'], 'list_b' => ['bar'] },
      }
      source, target = create_source_and_target_xrefs(
        source_data: source_data, target_data: { 'bar' => { 'username' => 'bar' } })

      source.create_xrefs target, source_to_target_field: 'list_a'
      source.create_xrefs target, source_to_target_field: 'list_b'
      assert_equal [{ 'username' => 'bar' }], source.data['foo']['list_a']
      assert_equal [{ 'username' => 'bar' }], source.data['foo']['list_b']
      assert_equal [{ 'username' => 'foo' }], target.data['bar']['source']
    end
